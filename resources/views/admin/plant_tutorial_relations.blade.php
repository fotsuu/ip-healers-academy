...
<head>
    ...
    <title>Plant-Tutorial Relations - IP Healers Academy</title>
    ...
    <div class="main-content">
        ...
        <div class="content-area">
            <div class="content-header">
                <div>
                    <div class="content-title">Plant-Tutorial Relationships</div>
                    <div class="content-desc">Manage tutorial video links for plant preparation methods.</div>
                </div>
                <button class="add-btn" id="openAddModal">Add new</button>
            </div>
            <div class="search-bar">
                <input type="text" id="searchInput" placeholder="Search plant-tutorial relationships...">
            </div>
            <div class="table-scroll-wrapper">
                ...
                @forelse($relations as $relation)
                    <tr data-id="{{ $relation->id }}" data-plant-id="{{ $relation->plant_id }}" data-tutorial-id="{{ $relation->tutorial_id }}">
                        <td>{{ $relation->id }}</td>
                        <td>
                        ...
                        <td>
                            <div class="tutorial-info">
                                {{ optional($tutorials->firstWhere('id', $relation->tutorial_id))->title ?? 'Unknown' }}
                            </div>
                        </td>
            ...
            <!-- Add New Plant-Tutorial Relation Modal -->
            <div id="addRelationModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100vw; height:100vh; background:rgba(38,58,41,0.18); align-items:center; justify-content:center;">
                <div style="background:#fff; border-radius:12px; max-width:480px; width:95vw; padding:32px 28px; box-shadow:0 4px 32px rgba(44,62,80,0.10); position:relative; max-height:90vh; overflow-y:auto;">
                    <div style="font-size:1.3rem; font-weight:700; color:#263a29; margin-bottom:18px;">Add New Plant-Tutorial Relation</div>
                    ...
                    <form id="addRelationForm">
                        ...
                        <div style="margin-bottom:16px;">
                            <label style="font-weight:600; color:#295024;">Tutorial</label><br>
                            <select name="tutorial_id" style="width:100%; padding:10px 12px; border:1.5px solid #e3e7df; border-radius:6px; font-size:1rem; background:#f7f8f5; color:#263a29;">
            ...
            <!-- Edit Plant-Tutorial Relation Modal -->
            <div id="editRelationModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100vw; height:100vh; background:rgba(38,58,41,0.18); align-items:center; justify-content:center;">
                <div style="background:#fff; border-radius:12px; max-width:480px; width:95vw; padding:32px 28px; box-shadow:0 4px 32px rgba(44,62,80,0.10); position:relative; max-height:90vh; overflow-y:auto;">
                    <div style="font-size:1.3rem; font-weight:700; color:#263a29; margin-bottom:18px;">Edit Plant-Tutorial Relation</div>
                    ...
                    <form id="editRelationForm">
                        ...
                        <div style="margin-bottom:16px;">
                            <label style="font-weight:600; color:#295024;">Tutorial</label><br>
                            <select id="editRelationTutorial" name="tutorial_id" style="width:100%; padding:10px 12px; border:1.5px solid #e3e7df; border-radius:6px; font-size:1rem; background:#f7f8f5; color:#263a29;">
                                <option value="">Select Tutorial</option>
                                @foreach($tutorials as $tutorial)
                                    <option value="{{ $tutorial->id }}">{{ $tutorial->title }}</option>
                                @endforeach
            ...
            <div class="delete-modal" id="deleteModal">
                <div class="delete-modal-content">
                    ...
                    <div class="delete-modal-message">Are you sure you want to delete this plant-tutorial relationship? This action cannot be undone.</div>
            ...
            <script>
                ...
                document.getElementById('addRelationForm').onsubmit = function(e) {
                    ...
                    fetch('/admin/plant-tutorial-relations', {
                        method: 'POST',
                        headers: {
                ...
                // Edit functionality for plant-tutorial relations
                document.querySelectorAll('.edit-link').forEach(function(btn) {
                    btn.addEventListener('click', function(e) {
                        ...
                        var tutorialId = row.dataset.tutorialId;
                        ...
                        document.getElementById('editRelationTutorial').value = tutorialId;
                ...
                document.getElementById('editRelationForm').onsubmit = function(e) {
                    ...
                    saveBtn.textContent = 'Saving...';
                    var formData = new FormData(form);
                    fetch('/admin/plant-tutorial-relations/' + relationId, {
                        method: 'POST',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                ...
                // Delete functionality for plant-tutorial relations
                document.querySelectorAll('.delete-link').forEach(function(btn) {
                    btn.addEventListener('click', function(e) {
                        ...
                        document.getElementById('confirmDelete').onclick = function() {
                            fetch('/admin/plant-tutorial-relations/' + currentRelationId, {
                                method: 'POST',
                                headers: {
                                    'X-Requested-With': 'XMLHttpRequest',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    document.getElementById('deleteModal').style.display = 'none';
                                        showCustomAlert('success', 'Success', 'Plant-tutorial relation deleted successfully!');
                                } else {
                                    showCustomAlert('error', 'Error', 'Failed to delete plant-tutorial relation.');
                                }
                            })
                            .catch(() => {
                                showCustomAlert('error', 'Error', 'Failed to delete plant-tutorial relation.');
